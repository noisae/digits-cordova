...
(function(window){
    ...
    window.DigitsCordova = function DigitsCordova(consumerKey, callbackLink, options){
        ...
        if (typeof options === 'object'){
            ...
            options.hidden = options.hidden || defaultOptions.hidden;
            phone = options.phone || defaultOptions.phone;
        } else {
        ...
        var loadedCallback = function(){}; //If we something breaks in digits
        ...
        var callbacks = {
            ...
            sendCode: function(code, finishedCallback) {
                var autoFillCode = "document.getElementsByName('login_verification_challenge_response')[0].value='" + code + "';";
        ...
        this.open = function(){
            document.addEventListener("deviceready", function(){
                ...
                if (cordova.InAppBrowser){
                    ...
                    openWindow = cordova.InAppBrowser.open('https://www.digits.com/login?consumer_key=' + _this.consumerKey + '&host=' + callbackLink, "_blank", "location=no,hidden=" + hidden);
                    ...
                    openWindow.addEventListener('loadstop', function(event) {
                        if (options.autoFill){
                            if (options.smsIntercept){
                                smsIntercepting = smsInterceptor(callbacks.sendCode)
                            }
